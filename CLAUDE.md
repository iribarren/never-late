...
| `ui/widget/` | Home-screen widget (Glance). Colors come from the app's own schemes via `glance-material3`'s `ColorProviders` bridge, **not** from `MaterialTheme` — a Glance composition has no access to it. `WidgetColors.kt` holds the two mapping twins of `colorForUrgency` / `Priority.indicatorColor()` (change them together) plus hand-written `ColorProvider(day, night)` pairs for the roles the bridge doesn't carry: `calm`/`soon` and `outlineVariant`. Rounded corners come from a shape drawable + `ColorFilter.tint`, never `GlanceModifier.cornerRadius` (API 31+ vs `minSdk` 24). |
