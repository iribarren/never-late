...
| `ui/notification/` | Lock-screen notification + foreground service, plus the reminder scheduler, `ReminderReceiver`, `BootReceiver`/`BootRescheduleWorker`. Two notification channels: silent `tasks_pending`, alerting `task_reminders`. Two alarm kinds share that alerting channel — `ReminderKind.LEAD_TIME` (feature 09, N minutes before a deadline) and `ReminderKind.TIME_UP` (times-up-alert feature, fires the instant a task's `timerEndsAt`/`deadline` actually runs out, `min` of the two when both apply) — namespaced by `requestCodeFor(taskId, kind)` and a distinct `Intent.action` so the two alarms can never collide (see `docs/arquitectura.md`'s D1). `BootRescheduleWorker` is also enqueued from `NeverLateApplication.onCreate` (cold start), not just after `BOOT_COMPLETED`. On API 26+ the alerting `task_reminders` channel is created with `setBypassDnd(true)` (focus-mode-shielding, guarded by `isNotificationPolicyAccessGranted()`) so it can survive a Modo Foco session's Do Not Disturb — see the `ui/focus/` row below. |
| `ui/focus/` | Modo Foco: `FocusScreen`/`FocusViewModel` (the full-screen session + exit ritual, núcleo spec) plus its **device shield** (focus-mode-shielding spec) — three optional, independent measures chosen in the entry dialog (`TasksScreen.kt`'s `FocusEntryDialog`). `domain/focus/FocusShieldRestore.kt` holds the pure pieces: `FocusShieldOptions` (the three booleans, D11 defaults) and `shieldRestoreActionFor` (the six-row Do-Not-Disturb restoration decision, D5 — the only measure with a receipt, see `docs/arquitectura.md`'s D1/D4). `FocusShieldController`/`AndroidFocusShieldController` wrap the platform `NotificationManager` (`applyDoNotDisturb`/`restore`/`cancelBackstop`, injected via `di/StorageModule.kt`) behind the `ACCESS_NOTIFICATION_POLICY` **special access** declared in the manifest (never a runtime-permission dialog — checked via `isNotificationPolicyAccessGranted()`, offered via `SpecialAccessNotice` below, and never required: a denied/revoked grant simply means the measure is not applied, no crash, no blocked flow); `applyFocusShieldOnSessionStart` is the write-ahead start sequence, called from `AppNavHost.kt`'s `onFocusClick` right alongside the Activity-scoped `startLockTask()` attempt (screen pinning, D7 — friction, not a lock, verified via `getLockTaskModeState()` read back, never a receipt). `FocusShieldRestoreWorker` is the 12h `WorkManager` backstop **and** the immediate cold-start check enqueued from `NeverLateApplication.onCreate`; `MainActivity.onCreate` separately releases stray pinning on a cold start with no active session. Immersive system bars + `FLAG_KEEP_SCREEN_ON` are a `DisposableEffect` inside `FocusRoute` (never `MainActivity`, which stays limited to `enableEdgeToEdge()`) — composable-scoped, no undo machinery needed. `ui/components/SpecialAccessNotice.kt` (the shared *check → explain → send to Settings* special-access idiom, re-checked on `ON_RESUME`) is used by both this feature's Do-Not-Disturb row and `SettingsScreen`'s exact-alarm notice. |
